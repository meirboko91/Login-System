...
def is_valid_password(password):
    ...
    req = load_user_create_requierments(
        "CommunicationLTD/password_requirements.json")
    ...
    if request.method == 'GET':
        if(request.GET):
            ...
            user = UsersData.objects.raw(
                f"SELECT * FROM users_usersdata WHERE username = '%s'" % (username))
            if (len(list(user)) != 0):
                messages.info(
                    request, "Are you sure you do not have an account?")
                if (len(list(user)) > 1):
                    users = list(user)
            elif (not is_valid_password(data['password'])):
                messages.info(
                    request, "The password you entered does not meet the requirements, please try again.")
            elif not is_difference_password(data['password'], data['password_repeat']):
                messages.info(
                    request, "The passwords do not match, please try again.")
            else:
        ...
        if(u is not None):
            if(u.check_password(form.cleaned_data['existing_password'])):
                if(passwordNotInLasts(u, form.cleaned_data['new_password'])):
                    ...
                    messages.info(
                        request, "You already used this password, please try again.")
                    return render(request, "users/user_change_pwd.html", context=context)
            else:
                messages.info(
                    request, "The exising password is not correct, please try again.")
                return render(request, "users/user_change_pwd.html", context=context)
        else:
...
def forgot_pwd_view(request):
    ...
    if form.is_valid():
        ...
        if found_user.exists():
            hashed_code = hashlib.sha1(
                str(random.getrandbits(10)).encode('utf-8')).hexdigest()
...
def verify_code_view(request):
    ...
    if form.is_valid():
        ...
        found_user = UsersData.objects.filter(
            username=input_username, resetCode=input_code)
        if found_user.exists():
...
def reset_pwd_view(request):
    ...
    if form.is_valid():
        if not is_difference_password(form.cleaned_data['new_password'], form.cleaned_data['verify_password']):
            messages.info(
                request, "The passwords not match, please try again.")
            context = {
                'form': form,
